...
$conn = getconnect();
	...
	$stmt1->bind_param("s",$currentusername);
	$stmt1->execute();
...
if(isset($_POST['hiddensub']))
{
 ...
 //echo $_POST['hiddenvisibility'];
  ...
  $stmt = $conn->prepare("select CatergoryID from category where CategoryName = ?");
...
$time = date("Y-m-d H:i:s"); // to get the time into the thread 
 ...
 //echo $currentuserid."    ".$sub."    ".$catid."    ".$access."     ".$time;
  ...
  $stmt = $conn->prepare("insert into threaddetails(AuthorID, Subject, CatergoryID, Visibility, ThreadCreated) values (?,?,?,?,?)");
...
//  if($stmt->execute())
    ...
    $query = "insert into threadreply  (UserReplied, replycreatedtime, Body,replymarkedlat,replymarkedlong) values ($currentuserid,'$time','$body',$lat,$lng)";
  //echo $query;
  if($conn->query($query))
...
$query = "select ThreadID from Thread where replycreatedtime = '$time'";
...
$var3 = "Private";
  ...
  //  echo 'the user is from block '.$userblockid;
    ...
    if($resultgargi2->num_rows>0)
    {
     ...
     $stmt3->bind_param("ii",$threadvar,$row['UserID']);
